...
const FlipModal = () => {
  ...
  const [suggestion, setSuggestion] = useState<{ assunto: string; mensagem: string; categoria: string } | null>(null);
  ...
  return (
    <div 
      className="w-[95%] h-[500px] mx-auto max-w-[550px] perspective-1000"
    >
      <div 
      ...
      >
        ...
        >
          <div className={`w-full h-full transition-all duration-700 ${isFlipped ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        >
          <div className={`w-full h-full transition-all duration-700 ${isFlipped ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            {suggestion && (
